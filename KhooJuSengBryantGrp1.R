...
# model 4b using linear regression on our selected model: 7 variables
...
prp(m.4d.2, type=2, extra=104, nn=T, nn.box.col = 'light blue',digits=3, main="Pruned Classification Tree", cex = 0.5)
print(m.4d.2)
printcp(m.4d.2)
## Root node error: 0.34616
## m.4d.2 trainset error = 0.636 * 0.34616 = 0.2201 = 22%
## m.4d.2 test set error = CV error = 0.643 * 0.3461 = 22 = 22%
...
# 4e
timeframe1 <- subset(dividends.data.2e, 
                     subset=(dividends.data.2e$fyear=="2007"|dividends.data.2e$fyear=="2008"|dividends.data.2e$fyear=="2009"
                             |dividends.data.2e$fyear=="2010"))
timeframe2 <- subset(dividends.data.2e, 
                     subset=(dividends.data.2e$fyear=="2011"|dividends.data.2e$fyear=="2012"
                             |dividends.data.2e$fyear=="2013"))
timeframe3 <- subset(dividends.data.2e, 
                     subset=(dividends.data.2e$fyear=="2014"|dividends.data.2e$fyear=="2015"|dividends.data.2e$fyear=="2016"
                             |dividends.data.2e$fyear=="2017"))
...
prp(m.4d.1.pruned, type=2, extra=104, nn=T, nn.box.col = 'light blue',digits=5, main="Pruned Classification Tree")
print(m.4d.1.pruned)
...
prp(m.4d.2.pruned, type=2, extra=104, nn=T, nn.box.col = 'light blue',digits=5, main="Pruned Classification Tree", cex=0.6)
print(m.4d.2.pruned)
...
length(which(dividends.data.2e$ib <0)) #6772
length(which(dividends.data.2e$spi <0)) #10366
length(which(dividends.data.2e$ib <0 &dividends.data.2e$spi <0)) #3807
length(which(dividends.data.2e$ib <0 &dividends.data.2e$spi <0 & dividends.data.2e$dvc==0)) #3156 out of 3807, 82.9%
length(which(dividends.data.2e$ib <0& dividends.data.2e$dvc==0)) # 5961 out of 6772, 88.02%

# Only consider data with losses for qn 5
dividends.data.5 <- subset(dividends.data.2e, subset=(dividends.data.2e$ib<0))
nrow(dividends.data.5)#6772
m.5.1 <- lm(dvc ~  at + ib + spi + fyear  + exchg, data = dividends.data.5)
summary(m.5.1)
coef(m.5.1) # only the coefficients in the model

# Now only consider data with losses, and also spi <0
dividends.data.5.spi <- subset(dividends.data.2e, subset=(dividends.data.2e$ib<0&dividends.data.2e$spi<0))
nrow(dividends.data.5.spi)#3807
m.5.2 <- lm(dvc ~  at + ib + spi + fyear  + exchg, data = dividends.data.5.spi)
summary(m.5.2)
coef(m.5.2) # only the coefficients in the model
